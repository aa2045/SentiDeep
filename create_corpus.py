...
class CombineDataset():
    ...
    def combine_clean_xml(self):
        ...
        for i, row in self.grouped_df.iterrows():
            ...
            if soup.code:
                soup.code.decompose()  # Remove the code part present in questions
            # remove the paragraph and pre tags but keeps the text contained with the tags
            paragraph_tag = soup.p
            pre_tag = soup.pre
            clear_text = ''
            if paragraph_tag:
                clear_text = clear_text + paragraph_tag.get_text()
            if pre_tag:
                clear_text = clear_text + pre_tag.get_text()
            ...
            if soup.code:
                soup.code.decompose()  # Remove the code part present in answers
            # remove the paragraph and pre tags but keeps the text contained with the tags
            paragraph_tag = soup.p
            pre_tag = soup.pre
            clear_text = ''
